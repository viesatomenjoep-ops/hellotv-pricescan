...
drop policy if exists vendit_articles_read on vendit_articles;
create policy vendit_articles_read on vendit_articles
  for select using (current_user_role() is not null);
drop policy if exists vendit_articles_admin_write on vendit_articles;
...
drop policy if exists prices_sales_admin_read on prices;
...
drop policy if exists price_quarantine_admin_read on price_quarantine;
...
drop policy if exists rfid_tags_read on rfid_tags;
create policy rfid_tags_read on rfid_tags
  for select using (current_user_role() is not null);
drop policy if exists rfid_tags_link_insert on rfid_tags;
create policy rfid_tags_link_insert on rfid_tags
  for insert with check (current_user_role() in ('warehouse', 'admin'));
drop policy if exists rfid_tags_link_update on rfid_tags;
...
drop policy if exists rfid_tags_admin_delete on rfid_tags;
...
drop policy if exists alternative_overrides_read on alternative_overrides;
create policy alternative_overrides_read on alternative_overrides
  for select using (current_user_role() in ('sales', 'admin'));
drop policy if exists alternative_overrides_admin_write on alternative_overrides;
...
create or replace function approve_quarantine(p_id uuid)
returns void
language plpgsql
security definer
set search_path = public
...
create or replace function reject_quarantine(p_id uuid)
returns void
language plpgsql
security definer
set search_path = public
